...
def main(params, net_params):
    ...
    t0 = time.time()
    per_epoch_time = []
    early_stopping = EarlyStopping(patience=params['earlystopping_patience'], path='checkpoint1' + params['Dataset'] + 'AFP' + '.pt')
...
def func_to_be_opt_MO(hidden_dim, depth, decay, init_lr, lr_reduce_factor, dropout):
...
for i in range(len(dataset_list)):
    ...
    if not os.path.exists(file_path):
            os.mkdir(file_path)
    save_file_path = os.path.join(file_path, '{}_{}_{}_{}_{}'.format('SOPT_UCB_5', params['Dataset'], 'MPNN', splitting_list[i], time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
    ...
    hpbounds = {'hidden_dim': (16, 128), 'depth': (1, 6.99), 'decay': (0, 8.99),
             'init_lr': (-5, -1), 'lr_reduce_factor': (0, 10.99), 'dropout': (0, 8.99)}
    bounds_transformer = SequentialDomainReductionTransformer()
    mutating_optimizer = BayesianOptimization(f=func_to_be_opt_MO, pbounds=hpbounds, verbose=0, random_state=1000)
